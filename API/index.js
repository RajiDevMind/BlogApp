...
app.post("/register", async (req, res) => {
  ...
  try {
    const userDetails = await UserRequirements.create({
      username,
      email,
...
app.post("/post", uploadMiddleware.single("file"), async (req, res) => {
  ...
  if (!req.file) {
    res.json({ "No file": "No image inserted!" });
  } else {
...
app.put("/post", uploadMiddleware.single("file"), async (req, res) => {
  if (!req.file) {
    ...
    jwt.verify(token, secret, {}, async (err, info) => {
      ...
      const isAuthor =
        JSON.stringify(postDoc._id) === JSON.stringify(req.body.id);
      if (!isAuthor) {
